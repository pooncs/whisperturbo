...
class TranslationGUI:
    ...
    def _setup_gui(self) -> None:
        ...
        self._table = pn.widgets.Tabulator(
            ...
            layout=pn.Column(
                pn.pane.Markdown("### Live Transcriptions", styles={"font-weight": "bold"}),
                pn.pane.Markdown(""),
            ),
            layout=(400, 400),
            frozen_columns=["Time"],
    ...
    def _on_export_csv(self, event) -> None:
        try:
            default_path = f"transcription_{int(time.time())}.csv"
            self.fusion.export_csv(default_path)
            self._status_pane.object = f"**Status:** Exported to {default_path}"
    ...
    def _on_export_jsonl(self, event) -> None:
        try:
            default_path = f"transcription_{int(time.time())}.jsonl"
            self.fusion.export_jsonl(default_path)
            self._status_pane.object = f"**Status:** Exported to {default_path}"
        except Exception as e:
    ...
    def _on_export_srt(self, event) -> None:
        try:
            default_path = f"transcription_{int(time.time())}.srt"
            self.fusion.export_srt(default_path)
            self._status_pane.object = f"**Status:** Exported to {default_path}"
        except Exception as e:
